...
namespace JDFixer.Managers
{
    internal class JDFixerUIManager : IInitializable, IDisposable
    {
        ...
        public void Initialize()
        {
            ...
            // in this corpus -- no install on this machine ships CustomCampaigns.dll. The runtime
            // check already falls back to the base handler whenever the mod is absent, so this is
            // the same behaviour with the unbuildable half removed. Define CUSTOM_CAMPAIGNS to
            // restore it once that mod exists for 1.44.3.
            missionSelection.didSelectMissionLevelEvent += MissionSelection_didSelectMissionLevelEvent_Base;
#endif
